...
Authentication stages use the same private frame/document/origin/iframe-host binding when a provider renders native identifier, password or OTP controls in one visible eligible HTTPS child frame. The public manifest remains generic and frame URLs stay private. Explicit `Sign in`, `Log in` and supported verification actions may be guarded in the owning frame; generic `Continue`, `Next` and `Submit` remain fill-only unless the owner explicitly opts into one exact continuation on the `open`/`attach` call. That opt-in is consumed during binding and cannot authorize checkout actions.
