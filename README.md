...
The controller can publish a bounded stage containing typed `identifier`, `password`, `one_time_code`, email, phone, and other supported controls. It rebinds after React-style rerenders and mints a fresh request for the next stage. Auth controls may be discovered inside one visible, eligible HTTPS child frame (including form-less staged surfaces such as provider sign-in iframes); the frame origin, labels, selectors, and values remain private, while the Mini App receives only generic field metadata. A single explicit `Sign in`/`Log in` action may be guarded in its owning frame. If up to four eligible auth actions are visible, Hermes still publishes and fills the fields, then returns `action_selection_required` with only a bounded count; the owner answers with an ordinal and `select_auth_action` revalidates the exact private action before one click. `Continue`/`Next`/`Submit` remain ambiguous: a single ambiguous action is fill-only by default and requires explicit opt-in on the exact `open`/`attach` call. `Verify` is treated as auth only for one-time-code stages. Custom widgets, shadow controls, CAPTCHA, passkeys, MFA, and provider challenges remain human-owned.
