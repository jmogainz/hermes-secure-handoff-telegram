...
- Cross-origin child frames are considered only when they are HTTPS, visible/editable, semantically classified as supported secure/payment fields, and structurally contained by the top-level checkout scope. No provider host, selector, API key, or site name is hardcoded.
...
5. Add synthetic same-origin and cross-origin frame fixtures with visible card/expiry/CVC inputs and a top-level payment action; ensure hCaptcha-like hidden frames are ignored.
