...
- Auth stages may bind native identifier/password/OTP controls in one visible, structurally contained HTTPS child frame, including form-less staged provider surfaces with multi-token autocomplete hints. A single ambiguous Continue/Next/Submit remains fill-only unless the owner explicitly opts into one exact continuation on the open/attach call. If up to four eligible auth actions are visible, field publication/fill proceeds and returns a bounded action count; the owner selects an ordinal through a separate exact-frame revalidation step. The ordinal selector cannot authorize checkout, and labels/URLs/selectors remain private.
