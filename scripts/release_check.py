...
FORBIDDEN_MARKERS = (".vercel/", ".env", "browser_login_receipts", "CURRENT.md", "_REPORT.md")
...
def main() -> int:
    ...
    if "manifest_version: 1" not in manifest or "name: telegram-browser-login" not in manifest:
    ...
    if "hermes_telegram_browser_login/plugin.yaml" not in wheel_names:
    ...
    if "hermes_agent.plugins" not in entry_text or "telegram-browser-login = hermes_telegram_browser_login:register" not in entry_text:
